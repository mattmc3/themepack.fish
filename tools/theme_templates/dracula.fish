# name: '$name'
# preferred_background: $background
# url: $url

fish_color_autosuggestion $brblack
fish_color_cancel $red --reverse
fish_color_command $cyan
fish_color_comment $comment
fish_color_cwd $green
fish_color_cwd_root $red
fish_color_end $yellow
fish_color_error $red
fish_color_escape $magenta
fish_color_history_current --bold
fish_color_host $blue
fish_color_host_remote $blue
fish_color_keyword $magenta
fish_color_match --background=$blue
fish_color_normal $foreground
fish_color_operator $green
fish_color_option $yellow
fish_color_param $blue
fish_color_quote $yellow
fish_color_redirection $foreground
fish_color_search_match --background=$selection_background
fish_color_selection --background=$selection_background
fish_color_status $red
fish_color_user $cyan
fish_color_valid_path --underline
fish_pager_color_background
fish_pager_color_completion $foreground
fish_pager_color_description $brblack
fish_pager_color_prefix $cyan
fish_pager_color_progress $brblack
fish_pager_color_secondary_background
fish_pager_color_secondary_completion $foreground
fish_pager_color_secondary_description $brblack
fish_pager_color_secondary_prefix $cyan
fish_pager_color_selected_background --background=$selection_background
fish_pager_color_selected_completion $foreground
fish_pager_color_selected_description $brblack
fish_pager_color_selected_prefix $cyan
